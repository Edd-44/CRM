...
- Caps are entered as **pre-money** figures. A cap only bites once the pre-money passes `cap / (1 - discount)`, and that trigger point is fixed by the note while the round moves. Every note record shows whether its cap is binding, and if not, the pre-money at which it takes over and how much headroom is left; the Raise builder says how many of the selected notes are at their cap and where the first one triggers. All of it recalculates the moment the target raise or the equity offered changes.
